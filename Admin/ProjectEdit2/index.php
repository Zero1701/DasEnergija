...
include_once $_SESSION['BASE_PATH']  . '/Admin/View/editProjectTableView2.php';
...
$(document).ready(function() { 
	...
	jQuery('#odustani').click(function () {
		  ...
		  <?php //------------------------------------------------------------------------------------ ?>
				  jQuery('#spremi').click(function () {
				    ...
				    return false;
						...
						function spremi(){
							...
							jQuery.get("<?php echo $_SESSION['SITE_URL'];?>/Admin/Ajax/update2.php?" + $('#form2').serializeAnything(), {LID : <?php echo $_GET['id']; ?>}, function(data){
				  ...
				  function formePdf(brojForme){
						
						jQuery.get("<?php echo $_SESSION['SITE_URL'];?>/Admin/Ajax/pdfForma2.php", {forma: brojForme }, function(data){
				        
							jQuery('#pdf' + brojForme + 'prostorZaFormu').html(data);
				         });
		  };

		  function obrisiPdf(brojPdf)
		  {
			  jQuery('#pdf' + brojPdf + 'obrisi').click(function () {
					
			       
			      $("#previewpdf" + brojPdf).html('<input type="hidden" value="obrisano" id="pdf' + brojPdf + '" name="pdf' + brojPdf + '" />');
			      formePdf(brojPdf);
			    return false;
			   });
			  }
		  ...
		  obrisiPdf(16);
		  <?php //------------------------------------------------------------------------------------ ?>
				  function obrisizip(brojzip)
		  {
			  jQuery('#zip' + brojzip + 'obrisi').click(function () {
					
			       
			      $("#previewzip" + brojzip).html('<input type="hidden" value="obrisano" id="zip' + brojzip + '" name="zip' + brojzip + '" />');
			      formezip(brojzip);
			    return false;
			   });
			  }

		  function formezip(brojForme){
				
				jQuery.get("<?php echo $_SESSION['SITE_URL'];?>/Admin/Ajax/zipForma.php", {forma: brojForme }, function(data){
		        
					jQuery('#zip' + brojForme + 'prostorZaFormu').html(data);
		         });
};
		<?php //------------------------------------------------------------------------------------ ?>
							obrisizip(1);
